...
TOKEN_BOT_V2 = "8715967513:AAGWHv7vVa3bagp-SOk64JFnV3dMCBUIHG4"
ADMIN_ID = 8874242457
...
# --- TOMBOL (KEYBOARD) ---
def get_main_menu():
    keyboard = [[KeyboardButton("/stor📍"), KeyboardButton("/referral")]]
...
async def send_welcome_message(chat_id, user, context):
    ...
    text_welcome = (
        f"<b>💮 SELAMAT DATANG DI GMAILKU.ID</b>\n\n"
        f"Hallo {user.mention_html()} 👋🏼\n\n"
        f"Saya adalah —͟͟͞͞𝐍𝖆𝖙𝖎𝖔𝖓𝖆𝖑𝖞𝖙𝖎 𝐒𝖊𝖓𝖕𝖆𝖎⃟ 𒁍 Asisten Anda yang akan membantu mengelola proses penjualan akun Gmail Fresh.\n\n"
        f"<blockquote>"
        ...
        "</blockquote>\n"
        "<b>👇🏼 Silakan klik tombol di bawah keyboard Anda untuk memulai proses setoran!</b>"
    )
...
async def back_to_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Kembali Kehalaman Utama", reply_markup=get_main_menu())
    return ConversationHandler.END
...
async def process_wd_jumlah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.error(f"Gagal kirim WD ke Admin via Bot Utama: {e}")

    try:
        await bot_v2.send_message(chat_id=ADMIN_ID, text=text_admin_wd, parse_mode="HTML")
    except Exception as e:
        logging.error(f"Gagal kirim WD ke Admin via Bot V2: {e}")
...
async def stor_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_blacklisted(user.id, context):
        await update.message.reply_text("⛔ Akun Anda dalam daftar blacklist dan tidak dapat melakukan setoran.")
        return ConversationHandler.END
    ...
    if not await is_user_joined(context.bot, user.id):
        await update.message.reply_text("⚠️ Anda harus memverifikasi keanggotaan terlebih dahulu! Klik /start untuk memverifikasi.")
        return ConversationHandler.END

    text_stor = (
        "<b>💮 FORM SETORAN GMAIL FRESH</b>\n\n"
...
async def finalize_stor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text_success, parse_mode="HTML", reply_markup=get_main_menu())
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Yahh... Sayang Sekali, Storan Anda Di Batalkan....", reply_markup=get_main_menu())
    context.user_data.clear()
    return ConversationHandler.END
...
async def main():
    ...
    print("Bot Sedang Berjalan Tuan, Jika Ingin Nonaktifkan Bot Klik CTRL+C...")
